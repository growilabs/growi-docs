...
### リンクを知っている人のみ
  ...
  ::: warning
  v5.0 以降、この設定をしたページの取り扱いに注意が必要となります。  
  詳しくは [リンクを知っている人のみ](#リンクを知っている人のみ設定のページ取り扱いについて) についてをお読みください。
  :::
...
## リンクを知っている人のみ設定のページ取り扱いについて

v5.0 以降もこの設定がされたページは、基本的にパスリンクまたはパーマリンクを知るユーザーのみに表示されます。

しかし、この設定がされたページの配下にあたるようなパスで、この設定以外の権限（公開や特定グループのみなど）を持つページを作成すると、ページツリーに表示されてしまいます。

例えば、「リンクを知っている人のみ」が設定された `/Page1` が存在する状態で、「公開」権限が設定された `/Page1/Page2` を作成すると、ページツリーに `/Page1` と `/Page1/Page2` のどちらも表示されてしまいます。

「リンクを知っている人のみ」を設定する際は、ページのパスに十分に気を付けてください。
