...
v5.0 以降もこの設定がされたページは基本的に、パスリンクまたはパーマリンクを知るユーザーのみに表示されます。

しかし、この設定がされたページの配下にあたるようなパスでページを作成すると、ページツリーに表示されてしまいます。

例えば、「リンクを知っている人のみ」が設定された `/Page1` が存在する状態で、それ以外の権限が設定された `/Page1/Page2` を作成すると、ページツリーに `/Page1` と `/Page1/Page2` のどちらも表示されてしまいます。
