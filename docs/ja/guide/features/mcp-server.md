...
GROWI MCPへの接続には、次の3つの方法があります。

1. **AI ツール（スキル）でセットアップする（最短パス・推奨）**  
   セットアップ用のスキルが、UTCP Code-Modeの設定からGROWIへの接続確認までを対話的にガイドします。  
   [AI ツール（スキル）を使う](/ja/guide/features/ai-tools.html)のインストール手順から始めてください。
1. **UTCP Code-Mode 経由で手動セットアップする**  
   スキルを使わずに、推奨構成であるUTCP Code-Mode経由の接続を手動で設定します。  
   本ページの「UTCP Code-Mode 経由で接続する」セクションを参照してください。
1. **直接 MCP 接続する（フォールバック）**  
   UTCP Code-Modeを利用できない場合や、少数のツールを軽く試したい場合の方法です。  
   本ページの「直接 MCP 接続する」セクションを参照してください。

いずれの方法でも、GROWIのアクセストークンが必要です。まずは次のセクションの手順で取得してください。

## 事前準備: GROWIのアクセストークンを取得
...
4. 「トークンを作成」ボタンをクリックし、作成されたトークンをコピーして安全な場所に保存します。
...
## UTCP Code-Mode 経由で接続する

UTCP（Universal Tool Calling Protocol）は、MCP サーバーを効率的に利用するための中間レイヤーです。複数のツール呼び出しを1回のリクエストにまとめることで、トークン消費を抑えます。

GROWI MCP Server は30近くのツールを提供しているため、直接接続ではすべてのツールスキーマがMCPクライアントのコンテキストを常時消費します。  
UTCP Code-Mode 経由で接続することで、この消費を抑えられます。

また、UTCP Code-Mode を経由して GROWI MCP に接続することで、[AI ツール（スキル）](/ja/guide/features/ai-tools.html)を利用できるようになります。

::: tip
以下は手動で設定する場合の手順です。  
[AI ツール（スキル）](/ja/guide/features/ai-tools.html)のセットアップスキルを利用すると、これらの設定をスキルが対話的にガイドします。
:::
...
::: tip
`.utcp_config.json` をリポジトリ内など別の場所に配置したい場合は、後述の MCP クライアント設定で `UTCP_CONFIG_FILE` 環境変数にフルパスを指定することで対応できます。API トークンを含むため、`.gitignore` への追加を推奨します。
:::

環境変数（`GROWI_API_TOKEN_1` など）の説明は、後述の「直接 MCP 接続する」セクションを参照してください。複数の GROWI インスタンスを使用する場合も同様に `_2`, `_3` ... と番号を変えて追加します。

#### 2. MCP クライアントで UTCP を設定

MCP クライアント（Claude Desktop など）の設定ファイルに UTCP Code-Mode を MCP サーバーとして追加します。
...
設定を保存したら、MCP クライアントを完全に終了して再起動します。

#### 4. 接続確認

「GROWI に接続できますか？」と尋ねて、UTCP 経由で MCP が正しく動作しているか確認してください。

#### UTCP 接続時のトラブルシューティング
...
この警告は UTCP の内部ログが stdout に出力されることが原因です。ツールは正常に動作するため、そのまま使用してください。

##### ツールが見つからない

1. `.utcp_config.json` のパスが正しいか確認してください。
2. GROWI の URL に `https://` が含まれているか確認してください。
3. URL の末尾にスラッシュ（`/`）が含まれていないか確認してください。
4. MCP クライアントを再起動してください。

##### 起動時に isolated-vm のエラーが出る

`No native build was found for ... isolated-vm` というエラーが出る場合は、使用している Node.js のバージョンに対応するビルド済みバイナリが存在しません。  
Node.js の LTS 版（20 または 22 を推奨）をインストールして使用してください。

## 直接 MCP 接続する

UTCP Code-Mode を利用できない環境や、少数のツールを軽く試したい場合は、MCPクライアントにGROWI MCPサーバーを直接登録して接続できます。

::: tip
直接接続でもすべてのツールが動作しますが、全ツールのスキーマがMCPクライアントのコンテキストに常時読み込まれるため、トークン消費が大きくなります。  
継続的な利用には UTCP Code-Mode 経由の接続を推奨します。  
また、[AI ツール（スキル）](/ja/guide/features/ai-tools.html)の利用には UTCP Code-Mode 経由の接続が必要です。
:::

### 設定手順(Claude Desktopの場合)

#### 1. Claude DesktopでMCPを設定
