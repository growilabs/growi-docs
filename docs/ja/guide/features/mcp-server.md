...
今後、MCP自体の機能改善を行う中で、GROWI.cloudで管理されているGROWIに関しましては、ご契約プランに応じて利用できる機能が制限される可能性がございます。
:::
...
たとえば、「最新のフロントエンド技術について調査して、/tech/frontend-trends というページにまとめて」と指示するだけで、AIがウェブ検索を行い、収集した情報を整理してGROWIページを自動作成します。  
...
また、外部データ(CSV、APIレスポンスなど)を読み込んで、それを元にした一覧ページや詳細ページを自動生成することも可能です。  
...
設定ファイル(`claude_desktop_config.json`)を編集します。使用するGROWIの数に応じて、以下のいずれかの設定を行ってください。
...
- **GROWI_DEFAULT_APP_NAME**: このパラメータは省略可能ですが、設定することでClaude上で操作対象のGROWIを明示的に指定できます。識別名と同じ値、またはより分かりやすい名前を設定してください。単一アプリのみを設定する場合は不要です。
