...
[^*1]: ユーザーデータをバックアップ/リストアする場合、現在の'PASSWORD_SEED'を新しいGROWIシステムにセットすることを忘れないでください。さもなくば、ユーザーがパスワードでログインできなくなります。現在の'PASSWORD_SEED'はエクスポートされるZIPファイル中のmeta.jsonに保存されます。しかし、この方法には不確実な点があり、より確実なPASSWORD_SEEDの取得方法に関してGrowi.cloudへルプページにより詳しい説明がありますのでご参照ください。
