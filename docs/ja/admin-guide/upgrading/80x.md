...
GROWI v8.0 は、GROWI AI をエージェント駆動へ全面的に刷新し、新たに GROWI Vault を追加するメジャーアップデートです。あわせて、サーバーのメモリ使用量を最適化するための既定値の変更と、Elasticsearch v7 系のサポート終了が含まれます。
...
::: warning
従来の AI 連携設定は廃止され、設定方法が一新されました。AI 連携機能を利用している場合、アップグレード後はそのまま動作しません。新しい設定方法に沿った再設定が必要です。設定手順は [GROWI AI Agent のセットアップと管理](/ja/admin-guide/management-cookbook/growi-ai-agent.html) を参照してください。
:::
...
あわせて、AI チャットの入力欄で `@` に続けて文字列を入力するとページを検索でき、候補から選んだページをそのまま文脈として AI へ渡せるようになりました。使い方は [GROWI AI Agent](/ja/guide/features/growi-ai-agent.html) を参照してください。

<ContextualBlock context="docs-growi-org">

GROWI AI Agent では、利用する LLM プロバイダを OpenAI / Anthropic / Google / Azure OpenAI から選択できます。設定方法は [GROWI AI Agent のセットアップと管理](/ja/admin-guide/management-cookbook/growi-ai-agent.html) を参照してください。
...
Elasticsearch v7 系を利用しているシステムでは、アップグレード前に v8 系または v9 系へ移行してください。移行せずに環境変数 `ELASTICSEARCH_VERSION` に `7` 以下の値を設定したまま起動すると、全文検索機能の初期化に失敗し、検索が利用できない状態になります（サーバープロセス自体は起動しますが、ログにエラーが出力されます）。環境変数の詳細は [環境変数](/ja/admin-guide/admin-cookbook/env-vars.html) を参照してください。
...
- [x] Elasticsearch v7 系を利用しているか（利用している場合、アップグレード前に v8 系または v9 系への移行が必要）
