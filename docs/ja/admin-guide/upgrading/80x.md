...
Elasticsearch v7 系を利用しているシステムでは、アップグレード前に v8 系または v9 系へ移行してください。v7 系のまま（環境変数 `ELASTICSEARCH_VERSION` の値が `7` 以下）で起動すると、全文検索機能の初期化に失敗し、検索が利用できなくなります。サーバープロセス自体は起動しますが、ログにエラーが出力されます。環境変数の詳細は [環境変数](/ja/admin-guide/admin-cookbook/env-vars.html) を参照してください。
