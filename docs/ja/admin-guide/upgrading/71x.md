...
::: warning
v6.1.0〜v7.0.15 を経由してアップグレードする場合は、その範囲のバージョンで起動・常用したまま留まらないでください（Revision データのマイグレーションに関する不具合があります）。
詳細は [アップグレードの概要と v7 系への移行](/ja/admin-guide/upgrading/#避けるべき「途中下車」バージョン) を参照してください。
:::
