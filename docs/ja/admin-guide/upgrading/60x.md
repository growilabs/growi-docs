...
### [未実装(廃止検討中)] GROWI 独自記法のページリンク

v5.x 以前に存在していた以下の独自記法は 2023.01 時点で未実装であり、廃止を検討しています。
...
プレゼンテーション表示機能は 2023.01 時点で未実装です。今後再実装を予定しています。
