...
A・B の各表の読み方: 現在ご利用中のバージョンの**列**を上から下まで確認し、○ が付いている行の対応を実施してください。○ は「そのバージョンから最新バージョンへ上げるまでに対応が必要」であることを示します。各行冒頭の `[必須]` `[該当時]` は対応の性質を表し、`[該当時]` は記載された条件に該当する場合のみ対応が必要です。
