...
本ページは GROWI.cloud をご利用の場合（A）と OSS 版をセルフホストしている場合（B）で、読む節が分かれます。ご利用の形態に合わせて A または B を上から順に進めてください。各作業の具体的な手順は、A・B の表からリンクする「C. 手順詳細」にまとめています。

A・B の各表の読み方: 現在ご利用中のバージョンの**列**を上から下まで確認し、○ が付いている行の対応を実施してください。○ は「そのバージョンから最新バージョンへ上げるまでに対応が必要」であることを示します。各行冒頭の `[必須]` `[該当時]` は対応の性質を表し、`[該当時]` は記載された条件に該当する場合のみ対応が必要です。各表の行は `[必須]` を先に、その中では対応が必要となるバージョン数の多い順に並べています。
...
- GROWI.cloud では、MongoDB・Elasticsearch といったミドルウェア、コンテナイメージ、ビルド環境の管理を GROWI.cloud 側で行っています。そのため、GROWI のバージョンアップデート時にお客様側での対応をお願いする項目は、この後の A-2・A-4 の表に挙げたもののみとなります。
  - 表に無い項目についてご不明な点があれば、[お問い合わせページ](https://growicloud.atlassian.net/servicedesk/customer/portal/1) までご連絡ください。
- GROWI アプリのバージョン更新そのものは、GROWI.cloud のアプリ詳細画面から実施します。詳細は A-3 を参照してください。
  - 「B. GROWI OSS 版 をご利用中の場合」に記載した docker-compose・ソースビルドの手順は、OSS 版をセルフホストしている場合のものです。GROWI.cloud では不要です。

### A-1. 現在のバージョンを確認する

- まず、現在ご利用中の GROWI アプリのバージョンを確認します。GROWI.cloud のアプリ詳細画面の「バージョン」項目で確認できます（[バージョン](https://growi.cloud/help/ja/cloud/version.html)）。
- GROWI.cloud では、ご契約中のプランに応じたMongoDBデータのバックアップを自動で取得しています（[バックアップ](https://growi.cloud/help/ja/cloud/backup.html)）。お客様による事前バックアップの取得は不要です。

### A-2. GROWI.cloud でアップグレード前に実施すること

現在ご利用中のバージョンの列を確認し、○ が付いている行に対応してください。
...
| [該当時] [同時多人数編集（HackMD）を有効にしている場合: ビルトインエディタの同時多人数編集へ移行](#hackmd-連携からビルトインエディタの同時多人数編集へ移行する) | | | | | ○ | ○ | ○ | ○ | ○ |
...
- A-2 の対応をすべて済ませたうえで、GROWI.cloud のアプリ詳細画面から GROWI のバージョンを更新します。手順の詳細は [バージョン](https://growi.cloud/help/ja/cloud/version.html) を参照してください。
  - 自動バージョンアップを利用している場合は、自動で最新版へ更新されます。特定バージョンで固定している場合は、手動で更新してください。
  - ページ冒頭の警告のとおり、v6.1.0〜v7.0.15 を経由するアップグレードは行わないでください。
- アップグレード後は、[全文検索インデックスの再構築](#全文検索インデックスの再構築) を実施してください。
...
| [該当時] [記法・HTML の仕様変更に該当する既存ページの本文を書き換え](#記法と-html-の仕様変更に伴うページ本文の書き換え) | | | | | ○ | ○ | ○ | ○ | ○ |
...
「B-2. アップグレード前に実施すること」の対応をすべて済ませたうえで、GROWI 本体をアップグレードします。あわせて、B-1 に記載した MongoDB のバックアップを事前に取得してください。
...
[growi-docker-compose](https://github.com/growilabs/growi-docker-compose) で運用している場合は、次の手順で進めます。手順の詳細は [GROWI のアップグレード](https://docs.growi.org/ja/admin-guide/getting-started/docker-compose.html#growi-のアップグレード) を参照してください。
...
GROWI 本体のアップグレード**後**に対応が必要な項目を、分類ごとの表にまとめています。現在ご利用中のバージョンの列を確認し、○ が付いている行にすべて対応してください（列の意味は「B-2. アップグレード前に実施すること」と同じです）。
...
「A-2. GROWI.cloud でアップグレード前に実施すること」「A-4. GROWI.cloud でアップグレード後に実施すること」「B-2. アップグレード前に実施すること」「B-4. アップグレード後に実施すること」の各表からリンクで遷移してくる、対応ごとの詳細手順です。各項目の「対応タイミング」に、いつ実施すべきかを明記しています。
...
- **対応タイミング**: アップグレード後
- **対象バージョン**: 全バージョン共通（特定バージョンの変更に伴うものではありません）
...
1. インデックスが破損表示になっている場合は、正規化を利用できます。
