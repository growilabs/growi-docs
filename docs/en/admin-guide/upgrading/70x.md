...
::: tip
v7.0 included a major overhaul on the client side.
If you are upgrading to the v7 series now, we recommend running the latest v7 series (at least v7.4.0 or later) for stability, features, and security.
For an overview of the upgrade path, see [Upgrade Overview and Migration to v7](/en/admin-guide/upgrading/).
:::

::: warning
Versions from v6.1.0 through v7.0.15 (up to v7.0.15 in the v7.0 series) contain a bug in the migration of Revision data.
Do not start up and keep operating on a version in this range; proceed to v7.4.0 or later.
For details, see [Versions to Avoid Stopping On](/en/admin-guide/upgrading/#versions-to-avoid-stopping-on) and the [Dev Wiki page](https://dev.growi.org/69301054963f68dfcf2b7111).
:::
