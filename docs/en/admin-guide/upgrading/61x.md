...
For more information, see [README.md](https://github.com/weseek/growi/blob/master/README_JP.md) and [Development Startup](../../dev/startup-v5/start-development.html).
...
However, if the link contains a `mdcont-` prefix in the document, or if the link refers to a document or system outside of GROWI, it will not jump to the anchor when accessing the document.
