...
::: warning
Upgrading from v5.x or later to v6.x should be ensure stable operation, but the impact on users is not small. Please make sure to fully inform your users of the changes before and after the upgrade.
:::
...
### [Beta] Plugin Function

Added new plugin functionality.
A plugin list page will be published on the official website in the future.
...
The value of the "XSS (Cross Site Scripting) Prevention Settings", which could be configured from the markdown settings in the admin panel, has been changed.
...
By clicking the template icon (![template icon](/assets/images/template-icon.png)) on the edit screen, you can now apply fixed phrases to the page you are editing.
...
::: tip
The content of pages saved using the previous notation will not automatically change when the GROWI mainframe is upgraded.
...
The function to draw charts in blockdiag notation is not implemented as of 2023.01. We plan to re-implement it in the future.

### [Not Implemented(Under Consideration for Deprecation)] GROWI Original Notation Page Link

The following unique notations that existed before v5.x are not implemented as of 2023.01 and are being considered for deprecation.
...
### [Not Implemented] Presentation Display

The presentation display function is not implemented as of 2023.01. It will be re-implemented in the future.


## Things To Check Before Upgrading

- [x] Are you using the deprecated nocdn version regularly?
- [x] Are you whitelisting your own tags or attributes in your XSS Prevention Settings?
- [x] Are you using a deprecated custom HTML Header?
- [x] Are you using the deprecated Twitter OAuth 2 Authentication?
- [x] Draw.io (Diagrams.net) notation change
  - [x] Have you inform the users?
  - [x] Have you decided on a strategy for future rewrites of existing page content?
...
- [x] Deprecated or not implemented features
  - [x] Is it okay to upgrade to v6.0 without the following features available?
    - [x] blockdiag
    - [x] GROWI Original Notation Page Link
    - [x] "Create Bootstrap 4 Grid" modal
    - [x] Page Draft List
    - [x] Presentation Display
  - [x] Have you inform the users?
